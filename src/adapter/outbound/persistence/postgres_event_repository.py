...
class PostgresEventRepository(EventStorePort):
    ...
    def _find_existing_id(
        ...
        key_columns = ["title", "event_date", "region"]
        if not all(col in table_columns for col in key_columns):
            return None

        where_parts = ["title = %s", "event_date = %s", "region = %s"]
        params: list[object] = [
            record.get("title"),
        ...
        if "link_url" in table_columns:
            where_parts.append("COALESCE(link_url, '') = COALESCE(%s, '')")
            params.append(record.get("link_url"))

        query = SQL("SELECT id FROM {table} WHERE {where_clause} LIMIT 1").format(
            table=Identifier(self._table_name),
            where_clause=SQL(" AND ").join(SQL(part) for part in where_parts),
        )
